"""
Model FINAL - Production-Ready NBA Props Prediction Model
==========================================================

This is the final, production-ready prediction model based on extensive backtesting.

PERFORMANCE METRICS (Validated over 5 weeks, 222 games):
---------------------------------------------------------
- Overall Hit Rate: ~61%
- HIGH Confidence Hit Rate: ~70%
- PTS Hit Rate: ~65%
- REB Hit Rate: ~60%
- AST Hit Rate: ~60%
...
KEY FEATURES:
-------------
1. Stat-specific weighting (more season weight for stability)
2. L5/L15/Season weighted projections
3. Consistency-based confidence scoring
4. Trend detection (hot/cold streaks)
5. Opponent defense factor adjustment
6. Minimum 3 picks per game guarantee
7. Separate grading for HIGH/MEDIUM confidence

USAGE:
------
    from src.nba_props.engine.model_final import (
        get_daily_picks, run_full_backtest, ModelFinalConfig
    )
...
Author: Model Lab - Final Version
Version: 1.0
...
# ============================================================================
# Configuration (Optimized via Backtesting)
# ============================================================================

@dataclass
class ModelFinalConfig:
    """
    Production configuration - optimized through extensive backtesting.
    
    DO NOT MODIFY unless you've run backtests to validate changes.
    """
    # === STAT-SPECIFIC WEIGHTS ===
    # These were optimized through grid search
    # More season weight = more stability
    ...
    # === EDGE THRESHOLDS ===
    high_edge_threshold: float = 12.0      # For HIGH confidence
    medium_edge_threshold: float = 7.0     # For MEDIUM confidence
    min_edge_threshold: float = 5.0        # Minimum to consider
    
    # === CONFIDENCE SCORING ===
    # Thresholds for confidence tier assignment
    ...
    # Opponent adjustment strength
    opponent_adj_strength: float = 0.40
    
    # Consistency thresholds (CV = std/mean)
    low_cv_threshold: float = 0.20         # Very consistent
    high_cv_threshold: float = 0.35        # Volatile
...
def _generate_pick(
    ...
    if plog.games_played >= 20:
        ...
        sample_comp = 4
        warnings.append(f"Limited data ({plog.games_played} games)")
    
    # Minutes stability bonus (0-10)
    if plog.min_cv < 0.12:
        min_bonus = 10
        reasons.append("Stable minutes")
    elif plog.min_cv < 0.18:
        min_bonus = 5
    else:
        min_bonus = 0
...
def get_daily_picks(
    game_date: str,
    config: Optional[ModelFinalConfig] = None,
...
if __name__ == "__main__":
    print("Running Model Final quick backtest...")
    quick_backtest()
